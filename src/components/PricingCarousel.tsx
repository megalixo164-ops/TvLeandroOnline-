  ...
  return (
    <div className="relative w-full max-w-6xl mx-auto px-4 py-12" ref={containerRef}>
      ...
      <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between z-20 pointer-events-none px-4 xl:-mx-12">
        <button
          onClick={prevSlide}
          className="pointer-events-auto p-3 rounded-full bg-[var(--color-brand-cyan)]/10 border border-[var(--color-brand-cyan)]/30 text-[var(--color-brand-cyan)] hover:bg-[var(--color-brand-cyan)] hover:text-white transition-all duration-300 shadow-[0_0_15px_rgba(67,175,239,0.2)] hover:shadow-[0_0_25px_rgba(67,175,239,0.5)]"
      ...
      <div className="flex md:hidden justify-center gap-4 mb-6">
        <button
          onClick={prevSlide}
          className="p-2 rounded-full bg-[var(--color-brand-cyan)]/10 border border-[var(--color-brand-cyan)]/30 text-[var(--color-brand-cyan)]"
        >
          <ChevronLeft size={20} />
        </button>
        <button
        ...
        {plans.map((_, idx) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-all duration-300 ${
              idx === activeIndex 
                ? 'w-8 bg-[var(--color-brand-cyan)] shadow-[0_0_10px_rgba(67,175,239,0.5)]' 
  ...
  return (
    <div 
      className={`
        ...
        ${isActive 
          ? 'border-2 border-[var(--color-brand-cyan)] shadow-[0_0_30px_-5px_rgba(67,175,239,0.3)]' 
          : 'border border-white/10 opacity-80 hover:opacity-100'
        }
      `}
    >
      {plan.popular && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[var(--color-brand-cyan)] to-[var(--color-brand-cyan-light)] text-[#020b16] px-6 py-1.5 rounded-full text-sm font-black uppercase shadow-[0_5px_15px_rgba(67,175,239,0.4)] whitespace-nowrap tracking-wider z-20">
          MAIS POPULAR
        </div>
      )}
      ...
      <div className="text-center mb-6 mt-2">
        ...
        <p className="text-[var(--color-brand-cyan)] text-sm font-medium">{plan.subtitle}</p>
      </div>

      <div className="mb-8 flex items-baseline justify-center gap-1">
        <span className="text-2xl font-bold text-[var(--color-brand-cyan)]">R$</span>
      ...
      <ul className="space-y-4 mb-8 flex-1">
        {plan.benefits.map((benefit, i) => (
          <li key={i} className="flex items-center gap-3">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${isActive ? 'bg-[var(--color-brand-cyan)]/20 shadow-[0_0_10px_rgba(67,175,239,0.2)]' : 'bg-white/10'}`}>
              <Check className={`w-4 h-4 ${isActive ? 'text-[var(--color-brand-cyan)]' : 'text-gray-400'}`} strokeWidth={3} />
            </div>
            <span className="text-gray-300 font-medium">{benefit}</span>
          </li>
        ))}
        {plan.savings && (
          <li className="flex items-center gap-3 bg-[var(--color-brand-cyan)]/10 p-3 rounded-lg border border-[var(--color-brand-cyan)]/20 mt-4">
            <div className="w-6 h-6 rounded-full bg-[var(--color-brand-cyan)]/20 flex items-center justify-center shrink-0">
              <Check className="w-4 h-4 text-[var(--color-brand-cyan)]" strokeWidth={3} />
            </div>
            <span className="text-[var(--color-brand-cyan)] font-bold text-sm">{plan.savings}</span>
          </li>
        )}
      </ul>
      ...
      <a 
        ...
        className={`
          ...
          ${isActive 
            ? 'bg-gradient-to-r from-[var(--color-brand-cyan)] to-[var(--color-brand-cyan-light)] text-[#020b16] shadow-[0_0_20px_rgba(67,175,239,0.4)] hover:shadow-[0_0_30px_rgba(67,175,239,0.6)] hover:scale-105' 
